...
highR = 1000.
# Delta lambda / lambda = d ln lambda = 1 / R --> d log lambda = ln 10 / R
highRdloglam = 1 / highR / np.log(10)
# boundaries of bins:
highRwavearr = np.logspace(np.log10(0.60), np.log10(2.40), np.round(np.log10(2.40 / 0.60) / highRdloglam).astype(int))
highRwavecen = (highRwavearr[1:] + highRwavearr[:-1]) / 2

def process(filename):
   w, f = np.loadtxt(filename, unpack=True)
   w *= 1e6 # to microns
   binned = np.zeros(wavecen.size)
   highRbinned = np.zeros(highRwavecen.size)
   for i in range(wavecen.size):
      binned[i] = np.mean(f[(w > wavearr[i]) & (w < wavearr[i+1])])
   
   for i in range(len(highRbinned)):
      highRbinned[i] = np.mean(f[(w>highRwavearr[i]) & (w < highRwavearr[i+1])])

   return binned, highRbinned
...
sc = 0.8
fig = plt.figure(1, figsize=(8*sc,4*sc))
ax = plt.gca()
#ax.set_aspect(.00025)
...
for i, f in enumerate(['model.dat']):
   ...
   for bands in [[0.7, 2.4]]:
      ...
      plt.errorbar(w[inband], (simobs + noisereal)[inband] + offset, yerr=noise[inband], marker='o', color=col, linestyle='none',
          ms=2, label=('Henrietta (50 ppm)' if bands[0]==0.7 else None))
   ...
   plt.errorbar(lam, model - 300, yerr=noise, marker='s', color='r', linestyle='none', label='HST')
   plt.plot(wavecen, fl - 300, 'b')
