...
for i, f in enumerate(['model.dat']):
   ...
   offset = -900
   plt.plot(wavecen, fl + offset, color=colors[i], alpha=0.7, label="Model")
   ...
   for bands in [[0.89, 1.124], [1.153, 1.352], [1.466, 1.708], [1.921, 2.4]]:
      ...
      plt.errorbar(w[inband], (simobs + noisereal)[inband] + offset, yerr=noise[inband], marker='o', color='red', linestyle='none',
          ms=2, label=('MIRMOS' if bands[0]==0.89 else None))
   plt.errorbar([w[0]], [(simobs + noisereal)[0] + offset], yerr=[noise[0]], xerr=[0.05], marker='o', color='red', ms=5)
